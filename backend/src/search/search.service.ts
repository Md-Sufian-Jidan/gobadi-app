...
export class SearchService {
  ...
  async search(q: string, type?: SearchType): Promise<Partial<SearchResults>> {
    if (!q) {
    ...
    const [animals, marketplace, doctors, alerts] = await Promise.all([
      wantsAll || type === 'animals' ? this.animalsService.search(q) : Promise.resolve(undefined),
      wantsAll || type === 'marketplace' ? this.marketplaceService.search(q) : Promise.resolve(undefined),
      wantsAll || type === 'doctors' ? this.doctorsService.search(q) : Promise.resolve(undefined),
